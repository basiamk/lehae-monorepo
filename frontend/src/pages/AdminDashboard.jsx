...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    if (authLoading || !isAdmin || dataLoaded) return;
    const fetchData = async () => {
      try {
        const [ud, pd, rd] = await Promise.all([
          favoritesAPI.getUsers(),       // already returns array via extractList
          propertyAPI.getProperties(),   // already returns array via extractList
          favoritesAPI.getReports(),     // returns plain object — fine as-is
        ]);
        setUsers(ud);
        setProperties(pd);
        setReports(rd);

        // FIX: apply extractList to all direct axiosInstance calls too
        try {
          const vr = await axiosInstance.get('/api/verification/admin/');
          setVerifications(extractList(vr.data));
        } catch {}

        try {
          const sr = await axiosInstance.get('/api/support/');
          setSupportThreads(extractList(sr.data));
        } catch {}

        try {
          const ci = await axiosInstance.get('/api/contact-inbox/');
          setContactInbox(extractList(ci.data));
        } catch {}
  ...
  const openThread = async (landlordId) => {
    ...
    try {
      ...
      setThreadMessages(extractList(res.data));
      const sr = await axiosInstance.get('/api/support/');
      setSupportThreads(extractList(sr.data));
    } catch {}
  };
  ...
  return (
    <div className="page-enter min-h-screen bg-neutral-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div style={{ display:'flex',overflowX:'auto',gap:4,background:'#faf7f3',borderRadius:12,padding:4,marginBottom:20,border:'1px solid #ede8e0',WebkitOverflowScrolling:'touch',scrollbarWidth:'none' }}>
        ...
        <div style={{ background:'#fff',border:'1px solid #ede8e0',borderRadius:20,padding:28 }}>
          ...
          {tab === 'properties' && (
            <>
              ...
              <div style={{ background:'rgba(212,169,106,0.08)',border:'1px solid rgba(212,169,106,0.2)',borderRadius:10,padding:'10px 14px',marginBottom:20,fontSize:12,color:'#7a7060',fontFamily:"'DM Sans',sans-serif",lineHeight:1.6 }}>
                <strong style={{color:'#a8895f'}}>What you're approving:</strong> That this listing is genuine — real photos, plausible price. No ownership documents required.
              </div>
              {properties.length === 0 ? <p style={{ color:'#9c9080',fontSize:13 }}>No properties yet.</p> : (
                <div style={{overflowX:'auto'}}>
                  <table style={{ width:'100%',borderCollapse:'collapse',fontFamily:"'DM Sans',sans-serif",fontSize:13 }}>
                    <thead>
                      <tr>{['Property','Landlord','Status','Approved','Actions'].map(h=>(
                        <th key={h} style={{textAlign:'left',fontSize:10.5,fontWeight:600,letterSpacing:'0.07em',textTransform:'uppercase',color:'#9c9080',padding:'0 8px 12px 0',borderBottom:'1px solid #f3ede6'}}>{h}</th>
                      ))}</tr>
                    </thead>
                    <tbody>
                      {properties.map((p,i) => (
                        <tr key={p.id} style={{borderBottom: i < properties.length-1 ? '1px solid #f5f0e8' : 'none'}}>
                          ...
                          <td style={{padding:'12px 8px 12px 0',color:'#7a7060',verticalAlign:'middle'}}>{p.landlord_username}</td>
                          <td style={{padding:'12px 8px 12px 0',verticalAlign:'middle'}}>
                            <span className="ad-badge" style={{ background:p.status==='vacant'?'rgba(34,197,94,0.1)':'rgba(239,68,68,0.1)', color:p.status==='vacant'?'#22c55e':'#ef4444' }}>{p.status}</span>
                          </td>
                          <td style={{padding:'12px 8px 12px 0',verticalAlign:'middle'}}>
                            <span className="ad-badge" style={{ background:p.is_approved?'rgba(34,197,94,0.1)':'rgba(245,158,11,0.1)', color:p.is_approved?'#22c55e':'#f59e0b' }}>{p.is_approved ? 'Approved' : 'Pending'}</span>
                          </td>
                          <td style={{padding:'12px 8px 12px 0',verticalAlign:'middle'}}>
            ...
            <>
              ...
              {users.length === 0 ? <p style={{ color:'#9c9080',fontSize:13 }}>No users yet.</p> : (
                <div style={{overflowX:'auto'}}>
                  <table style={{ width:'100%',borderCollapse:'collapse',fontFamily:"'DM Sans',sans-serif",fontSize:13 }}>
                    <thead>
                      <tr>{['User','Role','Verified','Actions'].map(h=>(
                        <th key={h} style={{textAlign:'left',fontSize:10.5,fontWeight:600,letterSpacing:'0.07em',textTransform:'uppercase',color:'#9c9080',padding:'0 8px 12px 0',borderBottom:'1px solid #f3ede6'}}>{h}</th>
                      ))}</tr>
                    </thead>
                    <tbody>
                          ...
                          <td style={{padding:'12px 8px 12px 0',verticalAlign:'middle'}}>
                            <div style={{ display:'flex',alignItems:'center',gap:10 }}>
                              ...
                              <div>
                                ...
                                <div style={{ fontSize:11,color:'#9c9080' }}>{u.email}</div>
                              </div>
                            </div>
                          </td>
                          <td style={{padding:'12px 8px 12px 0',verticalAlign:'middle'}}>
                            <span className="ad-badge" style={{ background:u.profile?.is_landlord?'rgba(196,168,130,0.15)':'rgba(148,163,184,0.15)', color:u.profile?.is_landlord?'#a8895f':'#64748b' }}>{u.profile?.is_landlord ? 'Landlord' : 'Tenant'}</span>
                          </td>
                          <td style={{padding:'12px 8px 12px 0',verticalAlign:'middle'}}>
                            <span className="ad-badge" style={{ background:u.profile?.is_verified?'rgba(34,197,94,0.1)':'rgba(239,68,68,0.1)', color:u.profile?.is_verified?'#22c55e':'#ef4444' }}>{u.profile?.is_verified ? 'Verified' : 'Unverified'}</span>
                          </td>
                          <td style={{padding:'12px 8px 12px 0',verticalAlign:'middle'}}>
                            <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
          ...
          {tab === 'verifications' && (
            <>
              ...
              <div style={{ background:'rgba(212,169,106,0.08)',border:'1px solid rgba(212,169,106,0.2)',borderRadius:10,padding:'10px 14px',marginBottom:20,fontSize:12,color:'#7a7060',fontFamily:"'DM Sans',sans-serif",lineHeight:1.6 }}>
                <strong style={{color:'#a8895f'}}>What you're approving:</strong> That this is a real person with a valid ID. Once approved, their verified badge applies to all listings permanently.
              </div>
              {verifications.length === 0 ? <p style={{ color:'#9c9080',fontSize:13 }}>No verification requests yet.</p> : (
                <div style={{overflowX:'auto'}}>
                  <table style={{ width:'100%',borderCollapse:'collapse',fontFamily:"'DM Sans',sans-serif",fontSize:13 }}>
                    <thead>
                      <tr>{['Landlord','ID Number','Phone','ID Document','Status','Actions'].map(h=>(
                        <th key={h} style={{textAlign:'left',fontSize:10.5,fontWeight:600,letterSpacing:'0.07em',textTransform:'uppercase',color:'#9c9080',padding:'0 8px 12px 0',borderBottom:'1px solid #f3ede6'}}>{h}</th>
                      ))}</tr>
                    </thead>
                    <tbody>
                      {verifications.map((v,i) => (
                        <tr key={v.id} style={{borderBottom: i < verifications.length-1 ? '1px solid #f5f0e8' : 'none'}}>
                          <td style={{padding:'12px 8px 12px 0',fontWeight:500,verticalAlign:'middle'}}>{v.landlord_username}</td>
                          <td style={{padding:'12px 8px 12px 0',color:'#7a7060',verticalAlign:'middle'}}>{v.national_id_number || '—'}</td>
                          <td style={{padding:'12px 8px 12px 0',color:'#7a7060',verticalAlign:'middle'}}>{v.phone_number || '—'}</td>
                          <td style={{padding:'12px 8px 12px 0',verticalAlign:'middle'}}>
                            {v.id_document_url
                              ...
                              : <span style={{ fontSize:12,color:'#c4bdb4' }}>No document</span>}
                          </td>
                          <td style={{padding:'12px 8px 12px 0',verticalAlign:'middle'}}>
                            <span className="ad-badge" style={{ background:v.status==='approved'?'rgba(34,197,94,0.1)':v.status==='rejected'?'rgba(239,68,68,0.1)':'rgba(245,158,11,0.1)', color:v.status==='approved'?'#22c55e':v.status==='rejected'?'#ef4444':'#f59e0b' }}>{v.status}</span>
                          </td>
                          <td style={{padding:'12px 8px 12px 0',verticalAlign:'middle'}}>
                            <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                              {v.status === 'pending' && <>
            ...
            <>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:16,marginBottom:32}}>
                {[
                  ...
                  { label:'Pending Approval',  value:pendingProperties.length },
                ].map(({ label, value }) => (
                    ...
                    <div key={thread.landlord_id} onClick={() => openThread(thread.landlord_id)}
                      ...
                      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:3 }}>
                        <div style={{ display:'flex',alignItems:'center',gap:8 }}>
                          <div style={{ width:28,height:28,borderRadius:'50%',background:'linear-gradient(135deg,#d4a96a,#c4a882)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'#fff',flexShrink:0 }}>
                        ...
                        {thread.unread_count > 0 && (
                          <span style={{ minWidth:18,height:18,borderRadius:100,background:'#ef4444',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontWeight:700,color:'#fff',padding:'0 5px' }}>
                            {thread.unread_count}
                          </span>
                        )}
                      </div>
                ...
                {activeThread && (
                  <div style={{ display:'flex',flexDirection:'column' }}>
                    <div style={{ maxHeight:340,overflowY:'auto',display:'flex',flexDirection:'column',gap:8,paddingBottom:12 }}>
                      {threadMessages.length === 0 && <p style={{ fontSize:13,color:'#9c9080',padding:'20px 0',textAlign:'center' }}>No messages yet.</p>}
                      {threadMessages.map(msg => {
                        ...
                        return (
                          <div key={msg.id} style={{ display:'flex',justifyContent: fromAdmin ? 'flex-end' : 'flex-start' }}>
                            <div style={{ maxWidth:'78%',padding:'9px 13px',borderRadius: fromAdmin ? '14px 4px 14px 14px' : '4px 14px 14px 14px',background: fromAdmin ? '#1c1a17' : '#faf7f3',color: fromAdmin ? '#fff' : '#1c1a17',fontSize:13,lineHeight:1.6,fontFamily:"'DM Sans',sans-serif",border: fromAdmin ? 'none' : '1px solid #ede8e0' }}>
                              ...
                              <p style={{ margin:'3px 0 0',fontSize:10,opacity:0.5 }}>{new Date(msg.created_at).toLocaleString([],{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'})}</p>
                            </div>
                          </div>
                        );
                      })}
    ...
    <div style={{ position:'fixed',inset:0,zIndex:200,display:'flex',alignItems:'flex-start',justifyContent:'flex-end' }} onClick={onClose}>
      <div style={{ width:'100%',maxWidth:420,height:'100vh',background:'#fff',borderLeft:'1px solid #ede8e0',overflowY:'auto',padding:28,fontFamily:"'DM Sans',sans-serif" }}
        ...
        <div style={{ display:'flex',alignItems:'center',gap:14,marginBottom:24,paddingBottom:24,borderBottom:'1px solid #f3ede6' }}>
          ...
          <div>
            ...
            <div style={{ display:'flex',gap:6,marginTop:6,flexWrap:'wrap' }}>
              <span style={{ padding:'2px 9px',borderRadius:100,fontSize:11,fontWeight:500,background:u.is_landlord?'rgba(196,168,130,0.15)':'rgba(148,163,184,0.15)',color:u.is_landlord?'#a8895f':'#64748b' }}>{u.is_landlord ? 'Landlord' : 'Tenant'}</span>
        ...
        <div style={{ marginBottom:20 }}>
          ...
          {[{ label:'Email', value: u.email||'—' },{ label:'Phone', value: u.phone||'—' },{ label:'Name', value: u.full_name||'—' }].map(({ label, value }) => (
