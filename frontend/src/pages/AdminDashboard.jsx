...
const AdminDashboard = () => {
  ...
  // FIX: lookup so the Users tab can show honest verification state per
  // landlord instead of a blind "Verify" button that skipped document review
  // entirely. Keyed by landlord user id -> their latest submission (if any).
  const verificationByLandlordId = {};
  verifications.forEach(v => { verificationByLandlordId[v.landlord] = v; });
                  ...
                  <tbody>
                    {users.map((u,i) => (
                      <tr key={u.id} style={{borderBottom: i < users.length-1 ? "1px solid #f5f0e8" : "none"}}>
                        ...
                        <td>
                          {/* FIX: honest verification state — was a flat
                              Verified/Unverified badge with a blind-verify
                              button that skipped document review entirely */}
                          {u.profile?.is_verified ? (
                            <span style={{ padding:"3px 10px",borderRadius:100,fontSize:11,fontWeight:500, background:'rgba(34,197,94,0.1)', color:'#22c55e' }}>Verified</span>
                          ) : !u.profile?.is_landlord ? (
                            <span style={{ padding:"3px 10px",borderRadius:100,fontSize:11,fontWeight:500, background:'rgba(148,163,184,0.1)', color:'#64748b' }}>N/A</span>
                          ) : verificationByLandlordId[u.id]?.status === 'pending' ? (
                            <span style={{ padding:"3px 10px",borderRadius:100,fontSize:11,fontWeight:500, background:'rgba(59,130,246,0.1)', color:'#2563eb' }}>Docs pending review</span>
                          ) : verificationByLandlordId[u.id]?.status === 'rejected' ? (
                            <span style={{ padding:"3px 10px",borderRadius:100,fontSize:11,fontWeight:500, background:'rgba(239,68,68,0.1)', color:'#ef4444' }}>Docs rejected</span>
                          ) : (
                            <span style={{ padding:"3px 10px",borderRadius:100,fontSize:11,fontWeight:500, background:'#f3f4f6', color:'#9ca3af' }}>No documents submitted</span>
                          )}
                        </td>
                        <td>
                          <div style={{ display:'flex',gap:6,paddingTop:4 }}>
                            ...
                            {/* FIX: no more blind Verify button — pending/rejected
                                submissions route to the Verifications tab, where
                                the admin can actually see the ID document first */}
                            {u.profile?.is_landlord && !u.profile?.is_verified && verificationByLandlordId[u.id] && (
                              <button className="ad-action" style={{ background:'rgba(59,130,246,0.1)',color:'#2563eb',border:'1px solid rgba(59,130,246,0.2)' }} onClick={()=>setTab('verifications')}><Eye size={12}/>Review docs</button>
                            )}
